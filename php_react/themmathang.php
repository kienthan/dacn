...
if (
    ...
    if (filter_var($mamh, FILTER_VALIDATE_INT) ) {
            $insertMH = mysqli_query($db_conn, "Call THEMSP ($mamh,'$tenmh','$idphanloai','$idsale','$url',
                                                            '$idbrand','$mota','$idprice','$soluongton',
                                                            '$img','$tinhtrang',@kq)");
        if ($insertMH) {
            $last_id = mysqli_insert_id($db_conn);
            echo json_encode(["success" => 1, "msg" => "Thêm Thành Công Mặt Hàng", "id" => $last_id]);
        } else {
            echo json_encode(["success" => 0, "msg" => "Thất bại thêm Mặt Hàng"]);
        }
    } else {
