...
export class DataProvider extends Component {
        ...
        .then((data) => {
          if (data.success) {
            ...
            if (this.state.user.length > 0) {
              ...
              sessionStorage.setItem("token", JSON.stringify(data.khachhang));
  ...
  CTHD = (mahdInput) => {
    ...
    for (var i = 0; i < cart.length; i++) {
      var ctdonhang = {
        ...
        thanhtien:
          (1 - parseFloat(cart[i].saleoff)) * (cart[i].price * cart[i].count),
      };
  ...
  render() {
    const { products, cart, total, user, phanloai, saleoff, admin } =
      this.state;
    const {
      addCart,
      reduction,
